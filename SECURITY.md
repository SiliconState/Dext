...
- Optional pack `runtime.json` v1 helpers are executable code and require a separate activation approval; approval profile `never` disables them. Manifest reads are no-follow regular files capped at 256 KiB, commands must resolve to regular executable files within the canonical pack root, and tool names/schemas/risk plus request/response/state/effect/continuation sizes are bounded and fail closed. Activation, idle, and declared read tools run under read-only confinement; write/danger tools retain normal approval, sandbox, durable side-effect journal, and fail-closed Git checkpoint controls. Runtime helpers are one-shot process-group-contained subprocesses and receive no inherited credentials even when ordinary active-pack helpers declare them or `DEXT_INHERIT_TOOL_CREDENTIALS=1` is set. Runtime outputs/effects are privacy-redacted; saved state restores only after pack re-resolution, executable approval, and exact pack-source/manifest-hash checks. Same-user replacement after path validation remains within R-006.
