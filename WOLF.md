# Wolf project context

Wolf is a single-binary Rust coding agent. This file is auto-injected into the
...
- `benches/wolf_bench.rs` — criterion perf harness.

## Self-modification
When changing Wolf itself:
...
4. Reinstall the interactive binary after code changes: `cargo install --path . --force`.
   Never skip this; `target/release/wolf` is not what `wolf` on PATH invokes.
5. On Windows, failed install with “Access is denied” usually means another
   `wolf.exe` is running. Ask the user to close it; do not kill processes.
...
- Keep runtime clutter (`.wolf/`, `.pi/`, scratch logs, one-off docs,
  screenshots) ignored or deleted, not committed.

## Context and memory
- `WOLF.memory.md` is a compact prompt-facing cache; durable long-form project
  memory lives in `MEMORY.md` / pi-memory (`--project wolf`).
- Log durable decisions/findings to pi-memory, sync `MEMORY.md`, then keep only
  the distilled prompt-worthy cache in `WOLF.memory.md`.
...
- Windows Git Bash: TUI detection uses stdout terminal status; stdin is read
  only with `-p`.
- Windows file locks can block build/install while `wolf.exe` is running.
