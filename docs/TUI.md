...
- Structured slash output from `/help`, `/tools`, `/system`, packs, shelves, and sessions uses one display-cell-aware list renderer. Headers, sections, entries, metadata, empty states, omission notices, and `Use:` footers honor the terminal width minus a two-cell right gutter, capped at 100 columns; even sub-20-column terminals remain bounded. ANSI styling is emitted only for an interactive color-capable stdout with `NO_COLOR` unset and `TERM != dumb`. The TUI translates only supported SGR styling into Ratatui spans while removing other escape/control sequences and preserving normalized line boundaries.
