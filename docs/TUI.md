...
- The empty composer prompt is `❯ Type a request…   @ files · / commands`; typing, login, permission, paste-preview, and work-map behavior retain their existing paths. Slash completion includes `/project-extensions [status|reset]` for inspecting or reopening repository extension approval.
