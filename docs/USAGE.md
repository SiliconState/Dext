...
dext --eval [NAME] [--sandbox PROFILE]  run eval harness under the selected profile
```

Run `dext --help` for the exact options supported by the installed binary.

By default, Dext autosaves session state under a project-specific key, but a new invocation does not silently load the previous transcript. A no-argument run uses approval `always` and sandbox profile `danger-full-access`, equivalent to passing both flags explicitly. Full access means ambient authority inside the current host, container, VM, namespace, WSL environment, CI worker, remote shell, or service account; Dext does not attempt to escape an existing boundary. Use `--approval ask|auto-read|auto-write|never` and `--sandbox read-only|workspace-write` for stricter task-specific controls. Use `dext --resume` (or `/resume` in an interactive session) to restore prior state. Optional `recall.md` and Seat summaries are user-authored context, not autonomous project memory; `--no-session` disables durable session/log writes.
...
Dext includes a `local` provider for an OpenAI-compatible llama.cpp server at `http://127.0.0.1:8080`. Local behavior is selected by the built-in `local` provider identity or an OpenAI-compatible endpoint whose parsed URL host is `localhost` or a loopback IP; URL path/query substrings do not classify a cloud endpoint as local. Local providers use frugal context by default unless you explicitly select `standard`. On startup and provider/model switches, Dext probes llama.cpp (`/props`, `/slots`, then model endpoints) and uses the live runtime context window for whichever model alias the server exposes. You can select any local alias; no model-specific context value is built into the local provider. For reasoning-capable local profiles, Dext also consumes streamed `choices[].delta.reasoning_content` as thinking up to the 4 MiB aggregate stream bound, stores it in the session, replays only current-turn reasoning to llama.cpp across tool-result rounds, and counts that active replay toward context-pressure compaction while excluding prior-turn thinking. Verbose TUI and `stream-json` expose this raw local model output; plain console and final JSON omit it. Local requests explicitly send `chat_template_kwargs.enable_thinking`; `/effort off` disables template thinking, while every non-Off effort enables it and continues to use any declared `reasoning_effort` level. These local-only fields are not added to cloud Chat Completions requests. Start one server first, then select its alias:
...
dext --trust       # explicit alias for the default `approval=always`
dext --no-trust    # explicitly select the `ask` profile
...
Privacy redaction is enabled by default while user-readable files remain readable. Before approved hooks run, `DEXT_TOOL_INPUT` is privacy-redacted for both `pre_tool` and `post_tool`, and `DEXT_TOOL_RESULT` is redacted for `post_tool`. Before tool results enter model context or session logs, Dext replaces private-key blocks, real secret assignments, and explicitly labeled SSNs, payment-card numbers, and account identifiers. Ordinary unlabeled long numbers and decimal market/HTTP values are not treated as cards. A compact redaction note is appended only when a value was actually replaced. Set `DEXT_PRIVACY=strict` or use `/privacy strict` to additionally block sensitive-looking native read paths and hidden, ignored, symlink-following, or sensitive-glob search scopes, including compact/combined ripgrep forms such as `-g.env` and `-ig .env` plus wildcard-prefixed sensitive globs such as `*.env`. Set `DEXT_PRIVACY=0` or use `/privacy off` only when raw, unredacted local data is intentionally required. The default `danger-full-access` profile adds no kernel confinement and preserves ambient temp/cache configuration. On supported Linux/macOS hosts, optional `read-only` and `workspace-write` profiles preserve reads available to the Dext process user while confining writes: `read-only` permits only required scratch/device writes, and `workspace-write` additionally permits writes to the sandbox and common toolchain caches. Dext warns when an optional confined profile cannot apply kernel enforcement and continues with native path guards.

The complete release suite can run through the default full-access Dext process without a special escape profile. If an invocation explicitly selects `workspace-write` or `read-only`, shared `/tmp`, arbitrary PTYs, and Cargo install metadata may remain unwritable. An already-confined child cannot relax parent container, VM, namespace, seccomp, Seatbelt, service-account, or host policy.
...
```bash
DEXT_NO_TUI=1
DEXT_APPROVAL=always  # default approval policy; use ask for prompts
...
DEXT_SANDBOX_PROFILE=danger-full-access  # default: ambient environment authority
# DEXT_SANDBOX_PROFILE=workspace-write  # opt-in sandbox + scratch + toolchain cache writes
