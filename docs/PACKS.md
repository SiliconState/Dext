...
- Runtime activation is executable-code approval, separate from selecting `PACK.md`. Approval profile `never` disables it. Changing approval or sandbox policy revokes the active executable runtime, removes its dynamic grants/denials, and discards queued callbacks; invoking the pack again requires activation under the new policy. Activation and idle events run with read-only confinement even when the session allows writes.
