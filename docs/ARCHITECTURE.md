...
Lean schemas are the default to reduce prompt cost. Their descriptions carry only each tool's unique action/safety cue; argument structure remains in the stripped schema and cross-tool workflow stays in the compact system prompt. The default 13-tool ChatGPT payload is regression-capped below 3,600 serialized bytes. Full descriptions and schemas are available with `--tool-profile full` or `/tool-profile full`; `default` is treated as lean when parsing the env/CLI alias.
