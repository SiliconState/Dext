...
On Windows CI and release builders, the remaining release tests run serially to prevent shared process/environment fixtures from racing and poisoning their common lock. The scheduler-sensitive `fast_bash_command_returns_without_100ms_poll_tail` regression then runs alone. Its original `<90 ms` assertion remains unchanged; isolation prevents suite work from obscuring the process-wait regression it measures.
