...
- Rebuilt the public README around a concise product overview, real Dext frontend captures, direct quick start, plain safety boundaries, and links to focused references instead of duplicating the technical documentation. Added lightweight Linux/macOS and Windows installers that select a matching release, verify `SHA256SUMS`, install per-user, and use a documented exact-revision locked source fallback until the first tagged release; attestation-required mode fails closed instead of using an unattested source build. CI now parses and exercises installer help on both platforms, runs deterministic offline Unix installer regressions, and validates the documentation site.
