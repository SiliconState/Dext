...
- Local OpenAI-compatible llama.cpp reasoning is now first-class without changing frontier-provider behavior: reasoning-capable local routes consume bounded streamed `choices[].delta.reasoning_content` into Dext thinking blocks/events, show it through verbose TUI and `stream-json` while keeping console/final JSON clean, replay only current-turn reasoning across local tool-result rounds, and include that replay in active context-pressure accounting. Local-route detection now requires the built-in `local` identity or an actual loopback URL host instead of substring matches. Local main requests explicitly set `chat_template_kwargs.enable_thinking` (`false` for Off, `true` otherwise) while retaining declared `reasoning_effort` mappings; cloud Chat Completions neither ingest nor receive these local-only fields. Empty non-stream summary failures no longer echo raw provider JSON or hidden reasoning in diagnostics.

- Checkpoint recovery no longer bricks on read-only Unix mode drift: current-user-owned manifest/lock files with group/world read bits are repaired to `0600` at startup and before mutation, while symlinks, hard links, foreign ownership, and group/world writes remain fatal with observed modes and actionable remediation. New checkpoint-free `dext checkpoint repair`, `dext undo --repair`, and `/undo --repair` quarantine an invalid manifest byte-for-byte, recreate an empty private manifest, preserve hidden refs until prune, and keep quarantine evidence through ordinary retention. Doctor now names recoverable/fatal checkpoint state, distinguishes a valid manifest above its 256 KiB inspection ceiling from runtime corruption, and warns when mutating tools may be blocked. Mutation guards, native Git tools, checkpoints, and optional workspace confinement now share the canonical containing Git top-level as write scope while retaining the selected launch directory as command cwd/project context; this also normalizes Windows verbatim-path forms before scope comparison. `/sandbox` reports both and outside-scope errors point to the in-session remedy.

- Added `glm-5.3-flash` and its documented `glm-5.3-flash[1m]` Coding Plan variant to the built-in ZAI catalog with a 1M-token context, 131,072-token output limit, model-specific image-input capability, stable list pricing, and native `low`/`high`/`max` reasoning. GLM Flash requests retain the Anthropic-compatible ZAI route, always enable thinking, and map Dext Off/Minimal/Low to provider `low`, Medium/High to `high`, and XHigh/Max to `max`; compaction summaries also retain the required low-effort thinking shape. The built-in local alias lets `Qwen3.8` or `local/Qwen3.8` resolve to the exact llama.cpp server id `qwen3.8-27b-ud-q5_k_xl`, including from a clean catalog.

- A no-argument run now equals `dext --sandbox-profile danger-full-access --approval always`: Dext adds no confinement, preserves ambient temp/cache configuration, and auto-approves gated tools and host operations while retaining credential scrubbing, privacy redaction, bounded I/O, process cleanup, checkpoints, and side-effect journaling. Full access uses authority already granted by the current host, container, VM, namespace, WSL environment, CI worker, remote shell, or service account and never attempts escape. Optional approval and sandbox profiles remain available. Hooks, diagnostics, eval commands, native subprocess tools, and every pack-runtime event now consistently use the selected profile. The full-access process launcher avoids sandbox scratch/probes, while confined profiles retain private scratch and native enforcement. CI/release now require a native Windows ConPTY real-binary smoke test alongside the Unix PTY suite. The risk register now reflects ambient full-access and hostile-same-user behavior as accepted product boundaries, retains checkpoint restore as a reliability risk, and removes the intentionally supported Anthropic subscription compatibility route from open risks.
...
- The default approval profile is now `always`; `--no-trust`, `DEXT_APPROVAL=ask`, or an explicit `approval=ask` select prompting behavior. Current-run policy always wins over approval provenance stored in resumed sessions.
