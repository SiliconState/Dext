...
impl<B: Backend> Terminal<B> {
    ...
    pub fn resize(&mut self, area: Rect) -> Result<(), B::Error> {
        ...
        // Inline applications own the scrollback above the viewport. Clearing the whole display on
        // a horizontal shrink flashes that history and forces application-level replay; the
        // viewport clear below is sufficient because the next draw repaints it completely.
...
mod tests {
    ...
    // Inline shrink clears and repaints only the viewport. Content above it belongs to native
    // scrollback and must not flash or disappear.
    #[test]
    fn resize_inline_preserves_content_above_viewport_on_horizontal_shrink() {
        let mut backend = TestBackend::with_lines(["0000", "1111", "2222", "3333"]);
        backend
        ...
        let new_area = Rect::new(0, 0, 3, 4);
        terminal.resize(new_area).unwrap();

        assert_eq!(terminal.viewport_area, Rect::new(0, 2, 3, 2));
        terminal
            .backend()
            .assert_buffer_lines(["1111", "2222", "    ", "    "]);
    }
}
