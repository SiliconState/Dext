...
impl<B: Backend> Terminal<B> {
    ...
    pub fn clear(&mut self) -> Result<(), B::Error> {
        let original_cursor = match self.viewport {
            Viewport::Inline(_) => self.last_known_cursor_pos,
            Viewport::Fixed(_) | Viewport::Fullscreen => self.backend.get_cursor_position()?,
        };
        self.clear_viewport()?;
